...
#first way
my_df <- data.frame(my_col1 = vector1, my_col2 = vector2)
my_df

#second way
my_df <- data.frame(vector1, vector2, col.names = c("my_col1", "my_col2"))
my_df
```
It's up to you to see which syntax you prefer, but personally, I prefer to use the first method, which has the advantage of explicitly matching the name of each column to the associated data vector.


*Question 1* :

What happens if we forget to give column names?


```{r}
my_df <- data.frame(vector1, vector2)
my_df
```
...
What if we don't name the vectors either?

```{r}
my_df <- data.frame(c(1:10), c(2:11))
my_df
```
<details>
...
How can we rename all columns of a data.frame?

<details>
<summary>Answer</summary>
```{r}
colnames(my_df) <- c("new_col1", "newcol_2")
my_df
```
</details>
<br>
...
```{r}
my_empty_df <- data.frame(Genes_name = character(),
                            Expression = integer(),
...
For the rest of the course, we will use the Clinical_Cohort.csv table which you can find there XXX.
```{r}
...
If we select part of a dataframe, what is the type of this selection?
...
```{r}
table(clinical_data$NSCLC_type)
```
...
```{r}
subset(clinical_data, select = Age)
subset(clinical_data, select = c(Subject_number, Age))
```
...
if only one element -> impossible to select only one element with [`subset()`](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/subset).
...
```{r}
dim(adeno_clinical_data)
table(adeno_clinical_data$NSCLC_type)
```
We have 5 lines and 8 columns, we have 5 "adenocarcinoma" values in NSCLC_type columns and only "adenocarcinoma" values. And compared to previously observation of our data we had 5 adenocarcinoma too. So the subset is good!
...
colnames(clinical_data) != "Subject_number"
...
```{r}
clinical_data[,!(colnames(clinical_data) %in% c("Subject_number","Age"))]
```
...
```{r}
subset(clinical_data, NSCLC_type == 'adenocarcinoma')
subset(clinical_data, NSCLC_type %in% c('adenocarcinoma', 'squamous cell carcinoma'))
subset(clinical_data, Age > 60 & Age < 65)
subset(clinical_data, Age > 60 & NSCLC_type %in% c('adenocarcinoma', 'squamous cell carcinoma'))
```

We can combine the subset of rows and columns.
For example, to retrieve the IDs of patients who have 'adenocarcinoma':
```{r}
subset(clinical_data, NSCLC_type == 'adenocarcinoma', select = "Subject_number")
```
...
```{r}
library(dplyr)
filter(clinical_data, NSCLC_type == 'adenocarcinoma')
```
...
```{r}
filter(clinical_data, NSCLC_type %in% c('adenocarcinoma', 'squamous cell carcinoma'))
```
...
#compute the mean
mean(clinical_data[clinical_data$NSCLC_type == "adenocarcinoma", "Age"])

## other possible answer with R base
mean(clinical_data[clinical_data$NSCLC_type == "adenocarcinoma",]$Age)
mean(clinical_data$Age[clinical_data$NSCLC_type == "adenocarcinoma"])

## with subset()
mean(subset(clinical_data, NSCLC_type == 'adenocarcinoma', select = "Age")$Age)

## with filter()
mean(filter(clinical_data, NSCLC_type == 'adenocarcinoma')$Age)
...
```{r, eval = FALSE}
table(is.na(my_df$my_column))
```

### How to remove rows with any zeros
```{r, eval = FALSE}
# Using Base R
my_df_new <- my_df[apply(my_df!=0, 1, all),]

# Using dplyr
library(dplyr)
my_df_new <- filter_if(my_df, is.numeric, all_vars((.) != 0))
```

### How to remove rows with NA in all Columns
```{r, eval = FALSE}
my_df_new <- my_df[rowSums(is.na(my_df)) != ncol(my_df), ]
```

### How to remove rows with NA in at least One Column
```{r, eval = FALSE}
my_df_new <- my_df[complete.cases(my_df), ]
```

### How to remove duplicated rows
```{r, eval = FALSE}
# Using Base R
my_df_new <- my_df[!duplicated(my_df)]
# Using dplyr
library(dplyr)
my_df_new <- my_df %>% distinct()
```
...
my_df_new <- my_df %>% distinct(my_column, .keep_all = TRUE)
# On 2 column named "my_column" and "my_awsome_column", with dplyr
my_df_new <- my_df %>% distinct(my_column, my_awsome_column, .keep_all = TRUE)
```
...
clinical_data <- rbind(clinical_data, new_patients)
...
clinical_data[,ncol(clinical_data)+1] <- Previous_radiotherapy_vector
head(clinical_data) # the new column is name V10
# print column names
colnames(clinical_data)
# rename the last column
colnames(clinical_data)[ncol(clinical_data)] <- "Previous_radiotherapy"
# print column names
colnames(clinical_data)
```
...
```{r}
colnames(clinical_data)[colnames(clinical_data) == "V8"] <- "Previous_radiotherapy"
```
...
cbind(clinical_data, Previous_immunotherapy = Previous_immunotherapy_vector) # the new column is named "Previous_immunotherapy"
...
```{r}
# print first lines
head(clinical_data_complement)
# dimensions
dim(clinical_data_complement)
# structure
str(clinical_data_complement)
```

Then, merge dataframes based on specific column:
```{r}
total_clinical_data <- merge(x = clinical_data, y = clinical_data_complement, by = "Subject_number")
head(total_clinical_data)
dim(total_clinical_data)
...
```{r}
total_clinical_data %>% filter(NSCLC_type == 'adenocarcinoma')
```
To assign the result to a variable (a new dataframe):
```{r}
filtered_clinical_data <- total_clinical_data %>% filter(NSCLC_type == 'adenocarcinoma')
```
...
```{r}
library(tidyr)
total_clinical_data <- total_clinical_data %>% separate(Mutation, c('Mutation_Gene', 'Mutation_Type'))
head(total_clinical_data)
```
...
```{r}
total_clinical_data$Mutation <- paste(total_clinical_data$Mutation_Gene, total_clinical_data$Mutation_Type, sep = "_")
...
```{r}
# sort the Age column in decreasing order
order(total_clinical_data$Age)

# sort total_clinical_data in decreasing order based on Age
total_clinical_data[order(total_clinical_data$Age), ]
```
...
```{r}
total_clinical_data[order(total_clinical_data$Age, decreasing = TRUE), ] #decreasing order
```

To order by several columns:
```{r}
total_clinical_data[order(total_clinical_data$Sex, total_clinical_data$Age, decreasing = TRUE), ] #decreasing order
```
...
```{r}
arrange(total_clinical_data, Age)
```
...
```{r}
arrange(total_clinical_data, desc(Age))
```
...
```{r}
mean(total_clinical_data[total_clinical_data$Sex == "M", "Age"])
mean(total_clinical_data[total_clinical_data$Sex == "F", "Age"])
```
...
```{r}
aggr_res <- aggregate(x = Age ~ Sex,              # Age is the value, Sex is the group 
...
```{r}
aggr_res <- aggregate(x = Age ~ Sex + NSCLC_type,     # Age is the value, Sex and NSCLC_type are the groups
                      FUN = mean,                     # the function you want to apply (here mean)
...
grpby_res <- total_clinical_data %>% # the dataframe
  group_by(Sex) %>%                  # Sex is the group 
  summarise(mean(Age))               # calculate mean on column Age in total_clinical_data
grpby_res
```
...
```{r}
grpby_res <- total_clinical_data %>%          # the dataframe
  filter(NSCLC_type == "adenocarcinoma") %>%  # only when patient has an adenocarcinoma
  group_by(Sex) %>%                           # Sex is the group 
  summarise(mean(Age))                        # calculate mean on column Age in total_clinical_data
grpby_res
```
...
```{r}
grpby_res <- total_clinical_data %>%  # the dataframe
  filter(Progression == "yes") %>%    # only where cancer is progressing
  group_by(Sex, NSCLC_type) %>%       # Sex and NSCLC_type are the groups
  summarise(mean(Age))                # calculate mean on column Age in total_clinical_data
grpby_res
```
...
More information here: https://en.wikipedia.org/wiki/Variant_Call_Format

Here we will work with the VCF avaiable here: "TO DO".

1) Load and see what the provided vcf named "Sample1.vcf.gz" looks like.
...
str(vcf1)
# add column names
colnames(vcf1) <- c("CHROM","POS","ID","REF","ALT","QUAL","FILTER","INFO","FORMAT","SAMPLE_INFO")
head(vcf1)
...
2) Load and see what the provided vcf named "Sample2.vcf.gz" looks like, then assemble the 2 files into one. Remember to keep the original patient information for each mutation.
...
str(vcf2)
# add column names
colnames(vcf2) <- c("CHROM","POS","ID","REF","ALT","QUAL","FILTER","INFO","FORMAT","SAMPLE_INFO")
head(vcf2)
...
nb_before_filtering <- vcf[,c("CHROM","POS","REF","ALT")] %>% distinct() %>% nrow()
# filtering
PASS_vcf <- subset(vcf, FILTER == "PASS")
# number of unique variants after filtering
nb_after_filtering <- PASS_vcf[,c("CHROM","POS","REF","ALT")] %>% distinct() %>% nrow()
...
4) What is the number of “PASS” variants in the X chromosome in patient Sample1.

<details>
<summary>Answer</summary>
```{r}
#get variants from the chromosome X in the patient Sample1
X_Sample1_vcf <- subset(PASS_vcf, CHROM == "X" & SAMPLE_NAME == "Sample1")
...
vector_PASS_variant <- paste(PASS_vcf$CHROM,PASS_vcf$POS,PASS_vcf$REF,PASS_vcf$ALT, sep = "_")
#get occurrences
occurrence_PASS_variant <- table(vector_PASS_variant)
#get variants where occurrence is 2
occurrenceis2_PASS_variant <- names(occurrence_PASS_variant)[occurrence_PASS_variant == 2]
# get number of variants where occurrence is 2
length(occurrenceis2_PASS_variant)
...
# get number of variants where occurrence is 2
nrow(occurrenceis2_PASS_variant)
